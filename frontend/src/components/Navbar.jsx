  ...
  return (
    <div className="w-full flex items-center justify-between px-6 py-3 sm:py-4 bg-background text-foreground border-b sticky top-0 z-50 border-1 border-black shadow-sm shadow-white">
      ...
      <ul className="flex justify-between gap-8">
        ...
        {user ? <li><NavLink to='/logout' className={({isActive})=>isActive? "text-gray-600 font-bold" : ""}>Logout</NavLink></li> : <li><NavLink to='/login' className={({isActive})=>isActive? "text-gray-600 font-bold" : ""}>Log In</NavLink></li>}
      </ul>
      <ThemeToggle />
    </div>
  );
